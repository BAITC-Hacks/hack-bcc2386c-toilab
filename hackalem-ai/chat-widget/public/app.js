...
if(config.localAI){document.querySelector(".chat-header div>span").textContent="HackAlem AI · Локальная модель";}
const demo=[
...
if(config.catalogMode!=="live")products();
...
const storageKey="hackalem-v2-"+(config.catalogMode||"demo");
...
function persist(){try{localStorage.setItem(storageKey,JSON.stringify({sessionId,history:history.slice(-40),cartUrl}))}catch{}}
...
async function send(text){
 ...
 const typing=document.createElement("div");typing.className="typing";typing.textContent=config.localAI?"Локальная модель обрабатывает вопрос…":"Консультант печатает…";$("#messages").append(typing);typing.scrollIntoView({block:"nearest"});
 const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),Number(config.requestTimeoutMs)||30000);
 try{
